...
<div class="bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="lg:hidden mb-6 sticky top-0 bg-gray-50 z-20 py-2 flex justify-end">
            <button id="mobile-filter-btn" onclick="toggleMobileFilters()" class="flex items-center gap-2 bg-white border border-gray-300 px-4 py-2 rounded-lg text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 active:bg-gray-100">
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <aside class="hidden lg:block w-64 shrink-0 bg-white border border-gray-200 rounded-xl p-5 shadow-sm sticky top-6 h-[calc(100vh-3rem)] overflow-y-auto sidebar-scroll">
            ...
            <main class="flex-1 min-w-0">
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h2 class="text-xl font-bold text-gray-900">All Products</h2>
                        <p class="text-gray-500 text-sm mt-1">Showing <span class="font-semibold text-gray-900">{{ $allProducts->total() }}</span> results</p>
                    </div>
                    <div class="flex items-center gap-3 w-full sm:w-auto">
                        <span class="text-sm text-gray-600 hidden sm:inline">Sort by:</span>
                        <select onchange="window.location.href=this.value" class="w-full sm:w-auto border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-violet-500 text-sm bg-white">
                            <option value="{{ request()->fullUrlWithQuery(['sort_by' => 'latest']) }}" {{ request('sort_by', 'latest') == 'latest' ? 'selected' : '' }}>Latest Products</option>
                            <option value="{{ request()->fullUrlWithQuery(['sort_by' => 'popular']) }}" {{ request('sort_by') == 'popular' ? 'selected' : '' }}>Popular</option>
                            <option value="{{ request()->fullUrlWithQuery(['sort_by' => 'price_low_high']) }}" {{ request('sort_by') == 'price_low_high' ? 'selected' : '' }}>Price: Low to High</option>
                            <option value="{{ request()->fullUrlWithQuery(['sort_by' => 'price_high_low']) }}" {{ request('sort_by') == 'price_high_low' ? 'selected' : '' }}>Price: High to Low</option>
                            <option value="{{ request()->fullUrlWithQuery(['sort_by' => 'oldest']) }}" {{ request('sort_by') == 'oldest' ? 'selected' : '' }}>Oldest Products</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    <div class="col-span-full py-16 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                        ...
                        <a href="{{ route('products.index') }}" class="inline-block mt-4 text-violet-600 hover:underline text-sm font-medium">Clear all filters</a>
                    </div>
                    @endforelse
                </div>
...
<div id="mobile-filter-drawer" class="fixed inset-0 z-50 lg:hidden translate-x-full transition-transform duration-300">
    <div id="mobile-filter-backdrop" class="absolute inset-0 bg-black/50 opacity-0 pointer-events-none transition-opacity" onclick="toggleMobileFilters()"></div>
        ...
        <form action="{{ route('products.index') }}" method="GET" class="flex flex-col flex-1 overflow-hidden">
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-6 sidebar-scroll">
                ...
                <div class="border-t border-gray-100 pt-4">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number" name="min_price" placeholder="Min" value="{{ request('min_price') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-500 focus:outline-none">
                        <span class="text-gray-400">-</span>
                        <input type="number" name="max_price" placeholder="Max" value="{{ request('max_price') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-500 focus:outline-none">
                    </div>
                </div>
            </div>
