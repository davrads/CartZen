                ...
                <div class="flex-1">
                    <div class="aspect-square w-full rounded-3xl overflow-hidden shadow-xl border border-gray-100 bg-white relative group">
                        ...
                        <div class="absolute top-4 left-4 z-20 flex flex-col gap-1">
                            <div class="bg-red-600 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-md animate-pulse">
                                Flash Sale
                            </div>
                            @if($flashEnd)
                            <div class="bg-black/70 text-white text-[10px] font-bold px-2 py-0.5 rounded-full backdrop-blur-sm">
                                Ends: {{ $flashEnd->format('M d, H:i') }}
                            </div>
                            @endif
                        </div>
    ...
    <section class="mt-16 lg:mt-24">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b pb-4">
            ...
            <a href="{{ route('categories.show', $product->category) }}"
                class="text-sm font-medium text-violet-600 hover:text-violet-700 flex items-center gap-2 self-start sm:self-auto">
                View More
        ...
        <form method="GET" action="{{ url()->current() }}" id="filterForm" class="bg-gray-50 p-4 sm:p-6 rounded-2xl border border-gray-200 mb-8 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                ...
                <div class="flex items-center gap-3">
                    <label class="flex items-center gap-2 cursor-pointer bg-white px-4 py-2.5 rounded-xl border border-gray-300 w-full hover:bg-gray-100 transition-colors">
                        <input type="checkbox" name="in_stock" value="1" {{ request('in_stock') ? 'checked' : '' }} onchange="document.getElementById('filterForm').submit()" class="rounded border-gray-300 text-violet-600 focus:ring-violet-500 w-4 h-4">
                        <span class="text-sm font-medium text-gray-700">In Stock Only</span>
                    </label>

                    @if(request()->hasAny(['sort_by', 'max_price', 'in_stock']))
                    <a href="{{ url()->current() }}" class="text-xs text-red-600 font-semibold underline hover:text-red-700 whitespace-nowrap">
                        Reset Filters
                    </a>
                    @endif
                </div>
