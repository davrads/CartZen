...
<form action="{{ route('place.order') }}" method="POST" id="checkoutForm">
    ...
    <div class="py-6 px-3 sm:px-6 lg:px-10">
        ...
        @if(session('error'))
            <div class="max-w-5xl mx-auto mb-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-xl relative text-sm font-semibold">
                {{ session('error') }}
            </div>
        @endif
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sm:p-6">
                        ...
                        @foreach($vendorsData as $index => $vendor)
                            <details class="store-row mb-3" open>
                                <summary class="flex items-center gap-2 cursor-pointer select-none mb-3">
                                    ...
                                    <span class="text-xs font-bold text-gray-700 tracking-wide uppercase">{{ $vendor['vendor_name'] ?? 'Store' }}</span>
                                ...
                                @foreach($vendor['items'] as $item)
                                    <div class="flex gap-3 bg-gray-50 rounded-xl p-3 mb-2">
                                        <div class="w-20 h-20 bg-white border border-gray-100 rounded-xl flex items-center justify-center p-2 shrink-0">
                                            <img src="{{ asset('storage/' . ($item['thumbnail'] ?? '')) }}" class="object-cover w-full h-full rounded-md">
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-semibold text-gray-800 leading-snug">{{ $item['name'] ?? 'Product' }}</p>
                                            <p class="text-xs text-gray-400 mt-0.5">Qty: {{ $item['quantity'] }}</p>
                                        </div>
                                        <div class="text-right shrink-0">
                                            <p class="mono text-sm font-bold text-gray-800">Rs. {{ number_format(($item['discounted_price'] ?? $item['price'] ?? 0) * $item['quantity']) }}</p>
                                        </div>
                                    </div>
                                @endforeach
                            </details>
                    ...
                    <div class="border-t border-gray-100 mt-4 pt-4 space-y-2.5">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="mono text-sm text-gray-700 font-medium">Rs. {{ number_format($cartSubtotal ?? 0) }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                    ...
                    <div class="flex justify-between items-center bg-violet-50 border border-violet-100 rounded-xl px-4 py-3.5 mt-4">
                        ...
                        <span class="mono font-extrabold text-violet-600 text-lg">Rs. {{ number_format($grandTotal ?? 0) }}</span>
                    </div>
